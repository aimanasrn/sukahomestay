...
export function updateBooking(id:string,status:LocalBooking["status"]){const all=readBookings().map(b=>b.id===id?{...b,status}:b);localStorage.setItem(key,JSON.stringify(all));window.dispatchEvent(new Event("sukahomestay-bookings"));}
